...
- `R/prep_public_receivers.R` reads the exact list from `data-raw/` (git-ignored),
  drops receiver names, snaps coordinates to a 0.05° grid (~5 km), and writes
  counts per grid cell × institution × status to `app/receivers_public.csv`.
...
3. Commit and push `app/receivers_public.csv`. GitHub Actions rebuilds and
